...
from argos.control_panel.cr_audit_closure import execute_cr10_level3_paper_candidate_audit  # noqa: E402


DEFAULT_OUTPUT = REPOSITORY_ROOT / "outputs" / "cr10_level3_paper_candidate"
...
def main() -> None:
    ...
    payload = execute_cr10_level3_paper_candidate_audit(REPOSITORY_ROOT, commit=commit)
    write(output / "cr10_result.json", payload)
    ...
    write(output / "overnight_phases.json", payload["overnightPhases"])
    write(output / "monitoring_plan.json", payload["monitoringPlan"])
    write(output / "morning_reconciliation_plan.json", payload["morningReconciliationPlan"])
    write(output / "financial_reconciliation_plan.json", payload["financialReconciliationPlan"])
    write(output / "shutdown_plan.json", payload["shutdownPlan"])
    write(output / "paper_candidate_qualification.json", payload["paperCandidateQualification"])
